...
describe('authSlice', () => {
  ...
  it('should handle login.rejected', () => {
    const action: AnyAction = { type: login.rejected.type, payload: 'Invalid password' };
    const state = authReducer(initialState, action);
    expect(state.loading).toBe(false);
  ...
  it('should handle logout', () => {
    const state = authReducer({ ...initialState, isAuthenticated: true, user: { id: 1, username: 'user' } }, logout());
